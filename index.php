	...
	<div id="app" class="app app-header-fixed app-sidebar-fixed">
		...
		<div id="content" class="app-content" style="min-height: 100vh; background: url(assets/img/cover/cover-scrum-board.png) no-repeat fixed; background-size: 360px; background-position: right bottom;">
			<div class="d-flex align-items-center mb-3">
				...
				<div class="ms-auto">
				<a href="#modal-task" data-bs-toggle="modal" id="add-task" class="btn btn-success btn-rounded px-4 rounded-pill"><i class="fa fa-plus fa-lg me-2 ms-n2 text-success-900"></i> Add Task</a>
				</div>
			</div>
			...
			<div class="row">
				...
				<div class="col-xl-4 col-lg-6">
					<div class="panel panel-inverse">
						<div class="panel-heading">
							<h4 class="panel-title">To do (<span id="to-do-tasks-count"><?=countTypes('1')?></span>)</h4>
							...
							<?php
								...
								foreach($tasks as $task):
									if($task['taskStatus'] == 'To Do'):?>
										<button type="button" data-bs-toggle="modal"  data-bs-target="#modal-task" class="col-sm-12 border-0 border-bottom bg-transparent d-flex align-items-start p-3" data-status="<?= $task["status_id"]?>" id="<?= $task["id"]?>" onclick="update('<?=$task['id']?>')">
											<div class="mt-1">
												<i class="far fa-question-circle text-success" ></i> 
											</div>
											<div class="card-body text-start p-0 ms-2">
												<div class="card-title fw-bold" id="title" data-title="<?= $task["title"] ?>"><?= $task["title"] ?></div>
												<div class="">
													<div class="card-subtitle text-black-50" id="datetime" data-datetime="<?= $task["task_datetime"] ?>"># <?=$counter++?> created in <?=$task["task_datetime"] ?></div>
													<div class="mb-2" id="description" title="<?= $task["description"] ?>"><?= substr($task["description"], 0, 100) . '...' ?></div>
												</div>
												<div class="d-flex">
													<span class="btn btn-primary" id="priority" data-prio="<?= $task["priority_id"] ?>"><?= $task["priorityName"] ?></span>
													<span class="btn btn-light text-dark ms-1" id="type" data-type="<?= $task["type_id"] ?>"><?= $task["typeName"] ?></span>
												</div>
											</div>
										</button>
										<?php 
				...
				<div class="col-xl-4 col-lg-6">
					<div class="panel panel-inverse">
						<div class="panel-heading">
							<h4 class="panel-title">In Progress (<span id="in-progress-tasks-count"><?=countTypes('2')?></span>)</h4>
							<div class="panel-heading-btn">
								<a href="javascript:;" class="btn btn-xs btn-icon btn-default" data-toggle="panel-expand"><i class="fa fa-expand"></i></a>
								<a href="javascript:;" class="btn btn-xs btn-icon btn-success" data-toggle="panel-reload"><i class="fa fa-redo"></i></a>
								<a href="javascript:;" class="btn btn-xs btn-icon btn-warning" data-toggle="panel-collapse"><i class="fa fa-minus"></i></a>
				...
				<div class="col-xl-4 col-lg-6">
					<div class="panel panel-inverse">
						<div class="panel-heading">
							<h4 class="panel-title">Done (<span id="done-tasks-count"><?=countTypes('3')?></span>)</h4>
							<div class="panel-heading-btn">
								<a href="javascript:;" class="btn btn-xs btn-icon btn-default" data-toggle="panel-expand"><i class="fa fa-expand"></i></a>
								<a href="javascript:;" class="btn btn-xs btn-icon btn-success" data-toggle="panel-reload"><i class="fa fa-redo"></i></a>
								<a href="javascript:;" class="btn btn-xs btn-icon btn-warning" data-toggle="panel-collapse"><i class="fa fa-minus"></i></a>
									...
									if($task['taskStatus'] == 'Done'):?>
										<button type="button" data-bs-toggle="modal"  data-bs-target="#modal-task" class="col-sm-12 border-0 border-bottom bg-transparent d-flex align-items-start p-3" data-status="<?= $task["status_id"]?>" id="<?= $task["id"]?>" onclick="update('<?=$task['id']?>')">
											...
											<div class="card-body text-start p-0 ms-2">
												...
												<div class="">
													<div class="card-subtitle text-black-50" id="datetime" data-datetime="<?= $task["task_datetime"] ?>"># <?=$counter++?> created in <?=$task["task_datetime"] ?></div>
													<div class="mb-2" id="description" title="<?= $task["description"] ?>"><?= htmlspecialchars(substr($task["description"], 0, 100)) . '...' ?></div>
												</div>
												<div class="d-flex">
													<span class="btn btn-primary" id="priority" data-prio="<?= $task["priority_id"] ?>"><?= $task["priorityName"] ?></span>
													<span class="btn btn-light text-dark ms-1" id="type" data-type="<?= $task["type_id"] ?>"><?= $task["typeName"] ?></span>
				...
				<form action="scripts.php" method="POST" id="form-task">
					...
					<div class="modal-body">
							...
							<div class="mb-3">
								...
								<span class="d-none text-danger" id="titleError">Title must not be empty or contains any number or special characters</span>
							</div>
							<div class="mb-3">
							...
							<div class="mb-3">
								...
								<span id="error-span" class="d-none text text-danger ms-2">Please select the task priority</span>
							</div>
							<div class="mb-3">
								...
								<span id="status-span" class="d-none text text-danger ms-2">Please select the task Status</span>
							</div>
							<div class="mb-3">
							...
							<div class="mb-0">
								...
								<span id="error-Des" class="d-none text text-danger ms-2">Please fill the description</span>
							</div>
